...
'''
本文件包含了zzsd引擎中的策略开发用模板，开发策略时需要继承ZzsdTemplate类。
'''
...
########################################################################
class ZzsdMuitiTemplate(object):
    """ZZSD策略模板"""
    
    # 策略类的名称和作者
    className = 'ZzsdMultiTemplate'
    ...
    def isScaleIn(self, stopPrice,direction):
        trade = self.ctaEngine.getSymbolLastTrade(self.vtSymbol)
        if trade:
    ...
    #----------------------------------------------------------------------
    def buy(self, price, volume,stopPrice, stop=False, isWeekReverse=False):
        """买开"""
        ##计算volume
        if volume == self.ctaEngine.FIX_SIZE_AUDO:
            if price == stopPrice:
                return []
            volume = self.ctaEngine.getSymbolVolume(self.vtSymbol,price ,stopPrice)

            if isWeekReverse == True:
                volume = volume/2

        preVolume = self.ctaEngine.getSymbolPrePosition(self.vtSymbol)
        ...
        if volume == 0:
            ...
            #jumpPrice = self.volumeMultiple * self.priceTick
            #price += jumpPrice  ##加上超价

        return self.sendOrder(CTAORDER_BUY, floor(price), volume, stop)
    ...
    def sell(self, price, volume, stop=False):
        ...
        return self.sendOrder(CTAORDER_SELL, floor(price), volume, stop)

    #----------------------------------------------------------------------
    def short(self, price, volume,stopPrice, stop=False , isWeekReverse=False):
        """卖开"""
        ...
        return self.sendOrder(CTAORDER_SHORT, floor(price), volume, stop)
    ...
    def cover(self, price, volume, stop=False):
        """买平"""
            ...
            #jumpPrice = self.volumeMultiple * self.priceTick
            #price += jumpPrice  ##加上超价
        return self.sendOrder(CTAORDER_COVER, floor(price), volume, stop)
    ...
    def cancelAllStop(self):
        """全部停止单"""
        self.ctaEngine.cancelAllStop(self.vtSymbol)

    def cancelAll(self):
        """全部撤单"""
        self.ctaEngine.cancelAll(self.vtSymbol)
...
########################################################################
class TargetPosTemplate(ZzsdMuitiTemplate):
    """
