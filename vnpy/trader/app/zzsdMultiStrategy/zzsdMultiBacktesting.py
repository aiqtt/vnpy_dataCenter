...
from .strategy.strategyZzsdMulti import  *
...
class BacktestingEngine(object):
    ...
    def __init__(self):
        ...
        self.weekSymbolFile = {}     #周线文件句柄  每天开始更新一下bar，小时线直接合成
        self.weekSymbolData = {}
        self.weekPreSymbolData = {} ##week 上一次bar data
    ...
    def runBacktesting(self):
        ...
        while True:
            ##### 1天1个循环
            #print( u"======开始新的一天=="+self.mainSymbol+"==="+curDate.strftime("%Y-%m-%d  %H:%M:%S") )

            #开始获取主力合约
            if not self.strategy.has_key(self.mainSymbol):
                print(u'切换合约==='+self.mainSymbol+curDate.strftime("%Y%m%d"))
                ...
                self.strategy[self.mainSymbol].setDayBar(self.loadDayData(self.mainSymbol,curDate ))
                self.strategy[self.mainSymbol].setHourBar(self.loadHourData(self.mainSymbol,curDate ))
                self.strategy[self.mainSymbol].setWeekBar(self.loadWeekData(self.mainSymbol,curDate))
                self.strategy[self.mainSymbol].setPriceTick(5, 1)
            ...
            self.cancleAllLimit(curDate.strftime("%Y%m%d")) ##每天清空限价单
            ...
            if self.mainSymbol != '' and self.strategy.has_key(self.mainSymbol) :
                mainBar = self.loadDayBar(self.mainSymbol,curDate)  ##日线数据等今天更新完再更新K的bar
                weekBar = self.loadWeekBar(self.mainSymbol,curDate)
                if mainBar:

                    if weekBar and self.weekPreSymbolData.has_key(self.mainSymbol):   ##周线数据保存的时间是周一的时间，需要等到1周执行完再更新bar
                        self.strategy[self.mainSymbol].updateWeekBar(self.weekPreSymbolData[self.mainSymbol])
                        self.weekPreSymbolData[self.mainSymbol] = weekBar

                    ##加载操作周期数据
                    initData3,isEnd  = self.loadOptBar(self.mainSymbol,"1min/", curDate)
                    if isEnd:
                        ...
                        self.strategy[self.mainSymbol].updateDayBar(mainBar)


            ##次主力

            if self.secSymbol != '' and self.strategy.has_key(self.secSymbol) :
                secBar = self.loadDayBar(self.secSymbol,curDate)
                weekBar = self.loadWeekBar(self.secSymbol,curDate)
                if secBar:

                    if weekBar and self.weekPreSymbolData.has_key(self.secSymbol):   ##周线数据保存的时间是周一的时间，需要等到1周执行完再更新bar
                        self.strategy[self.secSymbol].updateWeekBar(self.weekPreSymbolData[self.secSymbol])
                        self.weekPreSymbolData[self.secSymbol] = weekBar

                    #加载操作周期数据
                    initData2 , isEnd = self.loadOptBar(self.secSymbol,"1min/", curDate)
                    if isEnd:
                        ...
                        self.strategy[self.secSymbol].updateDayBar(secBar)


            if curDate > self.dataEndDate:
    ...
    def loadWeekBar(self,symbol,curDate):
        if not self.weekSymbolFile.has_key(symbol):
            self.weekSymbolFile[symbol] = open(self.FilePath+'week/'+self.symbol+".txt", 'r')

        dateTimeStr = curDate.strftime("%Y%m%d")

        while True:
            if not self.weekSymbolData.has_key(symbol):
                line = self.weekSymbolFile[symbol].readline()
                if line == '':
                    break
                self.weekSymbolData[symbol] = self.loadBarData(line)
            if self.weekSymbolData[symbol].date == dateTimeStr:
                return self.weekSymbolData[symbol]
    ...
    def crossLimitOrder(self):
        ...
        for orderID, order in self.workingLimitOrderDict.items():
            ...
            if buyCross or sellCross:
                ...
                if buyCross:
                    trade.price = order.price ##min(order.price, buyBestCrossPrice)
                    self.strategy[self.currentSymbol].pos += order.totalVolume
                else:
                    trade.price = order.price #max(order.price, sellBestCrossPrice)
                    self.strategy[self.currentSymbol].pos -= order.totalVolume
