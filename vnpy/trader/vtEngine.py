...
class MainEngine(object):
    ...
    #----------------------------------------------------------------------
    def connect(self, gatewayName, appName):
        """连接特定名称的接口"""
        ...
        if gateway:
            ...
            # 接口连接后自动执行数据库连接的任务
            self.dbConnect(appName)

    # ----------------------------------------------------------------------
    #本地运行
    def connect_local(self, gatewayName, appName):
        """连接特定名称的接口"""
        gateway = self.getGateway(gatewayName)

        if gateway:
            #加sleep，防止ctp的connect发生如下错误
            #ImportError: Failed to import _strptime because the import lockis held by another thread.
            #sleep(0.1)
            gateway.connect()

            # 接口连接后自动执行数据库连接的任务
            self.dbConnect_local(appName)
    ...
    def dbConnect(self, appName):
        ...
        if not self.dbClient:
            ...
            try:
                ...
                self.appDict[appName].loadMongoSymbol()
    ...
    #----------------------------------------------------------------------
    def dbConnect_local(self, appName):
        """连接MongoDB数据库"""
        if not self.dbClient:
            # 读取MongoDB的设置
            try:
    ...
    def dbInsert(self, dbName, collectionName, d):
        ...
        if self.dbClient:
            ...
            try:
                ...
                self.writeLog(e)
                self.logEngine.exception(e)
        else:
            self.writeLog(text.DATA_INSERT_FAILED)

    # ----------------------------------------------------------------------
    #出现新合约，给新合约在所有库中新建索引，
    def dbCreateIndex(self, dbName, symbol):
        db = self.dbClient[dbName]
        col = db[symbol]
        try:
