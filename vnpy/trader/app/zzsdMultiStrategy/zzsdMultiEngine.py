...
from .strategy.strategyZzsd import *
...
class ZzsdMultiEngine(object):
    ...
    def initStrategyBySymbol(self,symbol,setting):
        ...
        strategy.setDayBar(self.loadAllBar(DAY_DB_NAME,symbol ))
        if setting["strategyCycle"] == "60min":
            strategy.setHourBar(self.loadAllBar(MINUTE_60_DB_NAME,symbol ))

        if setting["strategyCycle"] == "30min":
            strategy.setHourBar(self.loadAllBar(MINUTE_30_DB_NAME,symbol ))

        if setting["strategyCycle"] == "5min":
            strategy.setHourBar(self.loadAllBar(MINUTE_5_DB_NAME,symbol ))
    ...
    def readDayData(self, dateTime,contract ):
        ...
        startDate = self.today - timedelta(10)  ##一般假期不会多于10天吧
        d = {'datetime':{'$gte':startDate}}
