...
from   vnpy.trader.app.zzsdStrategy.strategy.sarManager import *
...
########################################################################
class ZzsdStrategy(ZzsdTemplate):
    """结合ATR和RSI指标的一个分钟线交易策略"""
    className = 'ZzsdStrategy'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(ZzsdStrategy, self).__init__(ctaEngine, setting)
    ...
    def  handleZzsdStrategy(self,   askPrice1, bidPrice1):
        ...
        if self.getPos() == 0:
            if isLong == True:
                self.buy(askPrice1, self.fixedSize, self.hourSarManager.lowValue, False)

            if isLong == False:
                self.short(bidPrice1 , self.fixedSize,self.hourSarManager.highValue, False)
    ...
    def initDayCurrentBar(self,bar):
        ###更新当前bar， initDayBar加载的是历史bar，这个加载的是当前bar，因为今天还没结束，今天的bar还没生成
        ###这个接口一般实盘才会调用
        self.daySarManager.initCurrentBar(bar)
    ...
    #----------------------------------------------------------------------
    def initHourBar(self, initData):
        ##初始化小时bar数据
        self.hourSarManager.initBar(initData)

    def initHourCurrentBar(self,bar):
        ###更新当前bar， initHourBar加载的是历史bar，这个加载的是当前bar，因为今天还没结束，今天的bar还没生成
        self.hourSarManager.initCurrentBar(bar)
    #----------------------------------------------------------------------
